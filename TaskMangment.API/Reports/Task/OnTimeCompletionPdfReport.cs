...
namespace TaskMangment.API.Reports.Task
{
    public class OnTimeCompletionPdfReport : IDocument
    {
        ...
        public OnTimeCompletionPdfReport(List<EmployeeOnTimeReportDto> data)
        {
            _data = data;
        }
            ...
            container.Page(page =>
            {
                ...
                page.Header().PaddingBottom(10).Column(column =>
                {
                    column.Item().AlignRight()
                        .Text("أفضل الموظفين التزامًا بالمواعيد")
                        .FontSize(20)
                        .Bold();
                ...
                page.Content().PaddingTop(10).Table(table =>
                {
                    ...
                    table.Header(header =>
                    {
                        ...
                        header.Cell().Element(HeaderCellStyle).Text("منتهية في الوقت");
                        header.Cell().Element(HeaderCellStyle).Text("إجمالي المهام");
                        //header.Cell().Element(HeaderCellStyle).Text("نسبة الالتزام %");
                    });
                    ...
                    foreach (var row in _data)
                    {
                        ...
                        table.Cell().Element(c => DataCellStyle(c, bg))
                            .Text(row.OnTimeTasks.ToString());

                        table.Cell().Element(c => DataCellStyle(c, bg))
                            .Text(row.TotalTasks.ToString());

                        //table.Cell().Element(c => DataCellStyle(c, bg))
                        //    .Text($"{row.CommitmentPercentage}%");
                    }
                });
