...
namespace TaskMangment.Infrastructure.Services
{
 ...
 || await _commentRepo.GetAll(e => e.TaskId == id).AnyAsync();
     ...
     .ExecuteUpdateAsync(setters => setters
        ...
        public async Task<ApiResponse<bool>> CopyTaskAsync(CopyTaskDto dto, int userId)
        {
            await _uow.BeginTransactionAsync();

            try
            {
                var task = await _taskRepo.GetAll(t => t.Id == dto.TaskId)
                    .Include(t => t.Assignments)
                    .FirstOrDefaultAsync();
                ...
                var newAssignments = task.Assignments
                    .Where(a => a.IsActive)
                    .Select(a => new TaskAssignment
                    {
                        TaskId = newTask.Id,
                        EmployeeId = a.EmployeeId,
                        IsActive = true,
                ...
                await _assignmentRepo.AddRangeAsync(newAssignments);
                await _assignmentRepo.SaveChangesAsync();

                var employeeIds = newAssignments.Select(a => a.EmployeeId).ToList();

                await _eventDispatcher.PublishAsync(
                    new TaskAssignedEvent(newTask.Id, newTask.Title, employeeIds)
                );

                await _uow.CommitAsync();

                return ApiResponse<bool>.Ok(true, "Task copied successfully");
            }
            catch
            {
                await _uow.RollbackAsync();
